...
## Loans status by clusters

```{r fig.cap='Distribution of birth date', echo=FALSE, warning=FALSE, error=FALSE}

#Filter out lines without loan
frame_no_transaction %>% dplyr::filter(!is.na(status)) %>%
  #Group by cluster and status
  dplyr::group_by(cluster, status) %>% dplyr::summarise(amount_transactions = n()) -> prov

#Plot results
p <- ggplot(data = prov) +
  geom_raster(aes(x=cluster, y = status, fill = amount_transactions))

ggplotly(p)
```

## Loans status by duration

```{r fig.cap='Distribution of birth date', echo=FALSE, warning=FALSE, error=FALSE}

#Filter out lines without loan
frame_no_transaction %>% dplyr::filter(!is.na(duration)) %>%
  #Group by cluster and status
  dplyr::group_by(duration, status) %>% dplyr::summarise(amount_transactions = n()) -> prov

#Plot results
p <- ggplot(data = prov) +
  geom_raster(aes(x=duration, y = status, fill = amount_transactions))

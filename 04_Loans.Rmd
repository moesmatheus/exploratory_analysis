...
```{r fig.cap='Loan Status', echo=FALSE, warning=FALSE, error=FALSE}
...
We can also see that that the number of loans with some problem does not appear to have a growing tendency, although we can see that there is a decline in loans being issued by the bank in 1998 showing some room for improvement. Also in 1997 there was a great number of bad loans, problaby being the caus for the reduction of loans in 1998.

```{r fig.cap='Loan status by creation date', echo=FALSE, warning=FALSE, error=FALSE}


frame_no_transaction %>% 
  mutate( date = lubridate::floor_date(date.y, 'year') ) %>% 
  group_by(date, status) %>% 
  summarise(number = n()) %>% #dplyr::filter(operation!="") %>%
  ggplot() +
  geom_col(aes(x=date, y = number, fill = status)) +
  labs(y='Number',x = 'Year') -> p
...
```{r fig.cap='Loans by age at loan', echo=FALSE, warning=FALSE, error=FALSE}
...
By the Boxplot we can confirm the same conclusions with the addition that loans finished and not payed happened more with slighlty older people than the others.
...
```{r fig.cap='Loan Status by Loan amount', echo=FALSE, warning=FALSE, error=FALSE}

p <- ggplot(data = frame_no_transaction %>% dplyr::filter(!is.na(loan_id))) +
  geom_boxplot(mapping = aes(y  = amount, x = status, fill = status) )  +
  labs(x='Status',y = 'Loan Amount') +
  scale_y_continuous() + 
  theme(axis.text.x = element_text(size = 7))
...
p <- ggplot(data = frame_no_transaction %>% dplyr::filter(!is.na(loan_id)) %>% mutate(parcela = amount/duration)) +
  geom_boxplot(mapping = aes(y  = parcela, x = status, fill = status) )  +
  labs(x='Status',y = 'monthly payment') +
  scale_y_continuous() + 
  theme(axis.text.x = element_text(size = 7))

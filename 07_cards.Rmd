...
The growth in the number of credit cards has also been acompanied by a grow in the number of transactions done by credit card, mostly in the classic card.
...
As it happends with the amount of transactions there is also a spike in the use of credit cards every year in January, especially amongst the users of the classic card.
...
frame %>% 
  ...
  group_by(date, type.y) %>% 
  summarise(amount = sum(amount)/1e6) %>% 
  ggplot() +
  geom_col(aes(x=date, y = amount, fill = type.y)) +
...
frame %>% 
  dplyr::filter(operation=="credit card withdrawal") %>%
  group_by(account_id) %>% 
  summarise(amount = sum(amount)/1e3, type.y = first(type.y)) %>% 
  ggplot() +
